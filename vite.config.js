...
// }
// const getStableVersionHash = (item, rootDir) => {
//   if (item.type === 'chunk') {
//     const sourceCandidates = [item.facadeModuleId]
//     if (Array.isArray(item.moduleIds) && item.moduleIds.length === 1) {
//       sourceCandidates.push(item.moduleIds[0])
//     }
...
//         ? candidatePath
//         : path.join(rootDir, candidatePath)
//       if (!fs.existsSync(absolutePath)) continue
//       try {
//         const fileBuffer = fs.readFileSync(absolutePath)
//         return sha1(fileBuffer)
//       } catch {
//         // Ignore read failures and fallback to emitted bundle content hash.
//       }
//     }
//   }

//   return sha1(asBuffer(item.type === 'chunk' ? item.code : item.source))
// }
...
//           const hash = getStableVersionHash(item, rootDir)
...
//         const contentHash = getStableVersionHash(item, rootDir)
